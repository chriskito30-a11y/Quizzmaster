...
export function isFreeLimitError(error) {
  return Boolean(error && (error.code === "modulys/free-limit-reached" || String(error.message || "").toLowerCase().includes("limite de l’offre") || String(error.message || "").toLowerCase().includes("limite incluse")));
}
...
export async function assertCanCreateModuleEvent(moduleKey) {
  ...
  if (usage.eventsCreated >= limits.eventsPerMonth) {
    const error = new Error(`Limite incluse atteinte : ${limits.eventsPerMonth} création par mois pour ce module. Passez à une offre supérieure ou attendez le mois prochain.`);
